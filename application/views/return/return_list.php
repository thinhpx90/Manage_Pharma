								...
								{return_list}
									<tr>
                                        ...
                                        if($this->permission1->method('manage_invoice','read')->access()){ ?>
                                            <td><a href="<?php echo base_url().'Cretrun_m/invoice_inserted_data/{return_id}'; ?>">{invoice_id}</a></td>
                                        <?php } else{ ?>
                                            <td>{invoice_id}</td>
                                        <?php } ?>
										...
										<td>
											<center>
                                                <?php
                                                if($this->permission1->method('manage_invoice','read')->access()) { ?>
                                                   <a href="<?php echo base_url().'Cretrun_m/invoice_inserted_data/{return_id}'; ?>" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('invoice') ?>"><i class="fa fa-window-restore" aria-hidden="true"></i></a>
                                                <?php } ?>

                                                <?php
                                                if($this->permission1->method('manage_invoice','delete')->access()) { ?>
                                                    <a href="<?php echo base_url().'Cretrun_m/delete_retutn_invoice/{return_id}'; ?>" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('invoice') ?>"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                                <?php } ?>
